...
func (exs *EXS) GetZonesByKeyRanges(zonesPerRange int) []map[string][]*Zone {
	...
	sort.Strings(keys)
	klog.V(5).Infof("Zone key ranges: %v", keys)
	for _, k := range keys {
...
// IsDrumProgram attempts to detect if this is a drum program.
// Heuristics:
// - Most zones are single-note ranges (KeyLow == KeyHigh or KeyHigh - KeyLow <= 1)
// - Many zones have OneShot enabled
// - Zones are NOT contiguous/chromatic (would indicate a sampled instrument)
func (exs *EXS) IsDrumProgram() bool {
	klog.V(1).Infof("IsDrumProgram() called for %s with %d zones", exs.Name, len(exs.Zones))
	if len(exs.Zones) == 0 {
		klog.V(1).Infof("IsDrumProgram() returning false: no zones")
		return false
	}
	...
	oneShotCount := 0
	totalValidZones := 0
	keyNotes := make(map[int8]bool) // Track which key notes are used

	klog.V(1).Infof("Starting zone iteration (%d zones)", len(exs.Zones))
	for _, zone := range exs.Zones {
		// Skip zones without samples
		if zone.SampleIndex == -1 || int(zone.SampleIndex) >= len(exs.Samples) {
			continue
		}

		totalValidZones++

		// Check if this is a single-note or very narrow range
		keyRange := int(zone.KeyHigh) - int(zone.KeyLow)
		if keyRange <= 1 {
			singleNoteCount++
			// Record the key note for contiguity check
			keyNotes[zone.KeyLow] = true
		}

		// Check if OneShot is enabled
		if zone.OneShot {
			oneShotCount++
		}
	}

	if totalValidZones == 0 {
		return false
	}

	// Calculate percentages
	singleNotePercent := float64(singleNoteCount) / float64(totalValidZones)
	oneShotPercent := float64(oneShotCount) / float64(totalValidZones)

	// Check if single-note zones form a chromatic scale (contiguous)
	// This would indicate a sampled instrument, not drums
	isChromatic := false
	if singleNoteCount >= 12 { // At least one octave
		// Find the range of keys used
		minKey := int8(127)
		maxKey := int8(0)
		for key := range keyNotes {
			if key < minKey {
				minKey = key
			}
			if key > maxKey {
		...
		klog.V(1).Infof("Chromatic detection: minKey=%d, maxKey=%d, range=%d keys", minKey, maxKey, int(maxKey)-int(minKey)+1)

		// Count how many consecutive keys are present
		consecutiveKeys := 0
		maxConsecutive := 0
		// Use int for loop variable to avoid int8 overflow when iterating from 0 to 127
		for key := int(minKey); key <= int(maxKey); key++ {
			if keyNotes[int8(key)] {
				consecutiveKeys++
				if consecutiveKeys > maxConsecutive {
					maxConsecutive = consecutiveKeys
				}
			} else {
				consecutiveKeys = 0
			}
		...
		// If we have 12+ consecutive keys, it's likely chromatic (not drums)
		if maxConsecutive >= 12 {
			isChromatic = true
		}
	}

	klog.V(1).Infof("Drum detection: %d/%d single-note zones (%.0f%%), %d/%d one-shot zones (%.0f%%), chromatic: %v",
		singleNoteCount, totalValidZones, singleNotePercent*100,
		oneShotCount, totalValidZones, oneShotPercent*100, isChromatic)

	// If more than 50% of zones are single-note or very narrow, it's likely a drum kit
	// OR if more than 40% are one-shot, it's likely drums
	if singleNotePercent >= 0.5 || oneShotPercent >= 0.4 {
		klog.V(1).Infof("Detected drum program")
		return true
	}

	// Chromatic instruments should NOT be detected as drums
	// UNLESS they have 100% single-note zones, which is a strong indicator of drums
	if isChromatic && singleNotePercent < 1.0 {
		klog.V(1).Infof("Detected chromatic instrument (not drum)")
		return false
	}
...
func (exs *EXS) readChunks(r *bytes.Reader) error {
	...
	for i+84 < exs.Size {
		...
		switch chunkType {
		case headerChunk:
			klog.V(5).Infof("Chunk: %d (exs instrument chunk), size: %d", chunkType, header.Size)
			instrument, err := exs.readInstrument(r, i)
			if err != nil {
				return err
			}
			exs.Instrument = instrument
		case zoneChunk:
			if header.Size < 110 {
			...
			klog.V(2).Infof("Zone: %s, size: %d, keyLow: %d, keyHigh: %d, key: %d, sample: %d", zone.Name, header.Size, zone.KeyLow, zone.KeyHigh, zone.Key, zone.SampleIndex)
		case groupChunk:
		...
		case optionsChunk:
			...
			exs.Params = params
		case plistChunk:
			klog.V(5).Infof("Exs chunk type: %d (binary plist), size: %d - skipping", chunkType, header.Size)
		default:
...
func (exs *EXS) readZone(reader *bytes.Reader, pos int) (*Zone, error) {
	...
	zone := &Zone{
		...
		LoopEndRelease:  exsZone.LoopOpts&0x04 != 0,
		PlayMode:        exsZone.PlayMode,
		HasOutput:       exsZone.Opts&0x40 != 0,
	}
	return zone, nil
}
...
func (exs *EXS) readInstrument(reader *bytes.Reader, pos int) (*ExsInstrument, error) {
	_, err := reader.Seek(int64(pos), io.SeekStart)
	if err != nil {
		return nil, err
	}
	var instrument ExsInstrument
	if exs.BigEndian {
		err := binary.Read(reader, binary.BigEndian, &instrument)
		if err != nil {
			return nil, err
		}
	} else {
		err := binary.Read(reader, binary.LittleEndian, &instrument)
		if err != nil {
			return nil, err
		}
	}
	klog.V(5).Infof("Instrument: zones: %d, groups: %d, samples: %d", instrument.NumZones, instrument.NumGroups, instrument.NumSamples)
